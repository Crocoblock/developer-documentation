...
Allows to filter a list of a post statuses of the post that are retrieved during establishing a relationship. 

**Args:**
- `$statuses` - array - A list of post statuses. 
- `$object_name` - string - Post type. 
- `$relation` - object - Relation object.
...
```php
add_filter( 'jet-engine/relations/types/posts/get-items/post-statuses', function( $statuses ) {
	$statuses[] = 'private';
	return $statuses;
} );
```